...
import com.hyphenate.easeui.utils.SharedPreferencesUtilUi;
...
public class EaseNotifier {
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            String notifyText = " ";
            String title = " ";
            ArrayList<List<String>> list_service = (ArrayList<List<String>>) SharedPreferencesUtilUi.get(appContext,"chatservice","service");
            if(list_service!=null & list_service.size()>0){
                for(int i=0;i< list_service.size();i++){
                    for(int j=0;j< list_service.get(i).size();j++){
                        if(username.equals(list_service.get(i).get(j))){
                            if(i==0) {
                                notifyText = "日常训练师";
                            }else if(i==1){
                                notifyText = "行为分析师";
            ...
            msgIntent.putExtra("userTitle", title);
